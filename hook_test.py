    ...
    try:
        ...
        if 'ev' in json_msg:
            if json_msg['ev']=="C":
                ...
                # t.start()
                # r = Process(target=add_queue, args=(symbol, json_msg['t'], json_msg['a'], json_msg['b'], round_number,))
                # r.start()
                # add_queue(symbol,json_msg['t'],json_msg['a'],json_msg['b'],round_number)
                # add_db()
                database.add_db(symbol,json_msg['t'],json_msg['a'],json_msg['b'],round_number)
                # print("on_message ",time.time()-t0)
